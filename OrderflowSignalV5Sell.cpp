// =============================================================================
// OrderflowSignalV5Sell.cpp
// Sierra Chart ACSIL Custom Study  —  v5 SELL (Direct Cross-Chart Source Reads)
//
// Identical engine to OrderflowSignalV5. Differences, all in SetDefaults:
//   - Trigger defaults = the SHORT / BEARISH set (33 triggers) from
//     "OF-Triggers-For V5.docx"
//   - SG1/2/3 draw style = ARROW DOWN, width 6
//   - Trigger Position    = Above Candle
//   - Threshold / lookback / offset / sub-panel / fire-time defaults match the
//     tuned Buy instance from the user's screenshots (see SET DEFAULTS notes)
// -----------------------------------------------------------------------------
//
...
//
//   DEFAULTS ARE THE SHORT / BEARISH SET (33 triggers, 8 charts), taken from
//   "OF-Triggers-For V5.docx". The long/bullish half is in OrderflowSignalV5.
//
...
SCDLLName("OrderflowSignalV5Sell")

static const int OFS5_TAIL_BASE = 256;
static const int OFS5_TAIL_MAX  = 4096;
static const int OFS5_TAIL_EDGE = 8;

// -- Trigger defaults: SHORT / BEARISH set ----------------------------
// Source: "OF-Triggers-For V5.docx". 33 short triggers across 8 charts.
// The long/bullish half is loaded in OrderflowSignalV5 (the Buy study).
//
// SUBGRAPH INDEX IS 0-BASED HERE, the document is 1-BASED:
//   doc SG1 -> 0,  SG2 -> 1,  SG3 -> 2,  SG4 -> 3,
//   doc SG5 -> 4,  SG6 -> 5,  SG7 -> 6,  SG8 -> 7
//
// Chart numbers are the source charts from the document. If a chart
// number here does not match your chartbook, fix it in Study Settings —
// SG6 "Debug: Connected" must read 36 once every reference resolves.
//
...
// "Bearish" sheet — 36 triggers, 50 weight points, 7 charts. Regenerated
...
//   - chart #10 is 4.5 range, not 4.0, and uses ID9/ID1/ID25 (was ID69/ID72/ID40)
//   - chart #4 now reads ID1.SG32/36/38 (was ID50.SG1 / ID17.SG1)
//   - #2 "Short Volume Momentum" ID7.SG2 dropped (not in the sheet)
//   - #5 gains "Exhaustion Reversal Short" ID31.SG4 and "Delta Flip Down" ID33.SG7
//
...
static const s_TrigDef td[50] = {
    // ---- chart #2, 2.5 range ----
    { 2,  1,  1, 1, "Short Delta Smart Triggers" },  // ID1.SG2
    { 2,  6,  1, 1, "Bearish Delta Trap"         },  // ID6.SG2
    { 2,  6,  6, 1, "Bearish Reversal"           },  // ID6.SG7
    { 2,  7,  3, 1, "Short Exhaustion Reversal"  },  // ID7.SG4
    { 2, 10,  3, 2, "Short Slingshot 3-Bar"      },  // ID10.SG4
    { 2, 10,  5, 2, "Short FADE Trigger"         },  // ID10.SG6
    { 2, 10,  7, 1, "Short ABS"                  },  // ID10.SG8
    // ---- chart #4, 2.0 range ----
    { 4,  1, 31, 2, "Short SELL 8"               },  // ID1.SG32
    { 4,  1, 35, 2, "POCS Short"                 },  // ID1.SG36
    { 4,  1, 37, 1, "POC WAVE Sell"              },  // ID1.SG38
    // ---- chart #5, 3.5 range ----
    { 5, 24,  6, 1, "Short OF trigger A"         },  // ID24.SG7
    { 5, 24,  7, 1, "Short OF trigger B"         },  // ID24.SG8
    { 5, 30,  1, 1, "Short ABS"                  },  // ID30.SG2
    { 5, 31,  3, 1, "Exhaustion Reversal Short"  },  // ID31.SG4
    { 5, 33,  6, 1, "Delta Flip Down"            },  // ID33.SG7
    // ---- chart #7, 2.5 range ----
    { 7, 17,  0, 2, "Short SELL 10"              },  // ID17.SG1
    { 7, 19,  0, 1, "Bearish Vol Seq Div"        },  // ID19.SG1
    { 7, 40,  0, 1, "Delta Trap Signal Short"    },  // ID40.SG1
    { 7, 41,  0, 2, "Short POCS 10"              },  // ID41.SG1
    { 7, 50,  0, 1, "Short DeltaDown+"           },  // ID50.SG1
    { 7, 72,  0, 1, "Short POC WAVE"             },  // ID72.SG1
    // ---- chart #10, 4.5 range ----
    {10,  1,  1, 1, "Short POC Momentum"         },  // ID1.SG2
    {10,  9,  6, 2, "Short OF L1"                },  // ID9.SG7
    {10,  9,  7, 2, "Short OF L2"                },  // ID9.SG8
    {10, 25,  0, 1, "Delta Flip Down - Basic"    },  // ID25.SG1
    // ---- chart #13, 6.5 range ----
    {13,  1,  3, 2, "Short Slingshot 3-Bar"      },  // ID1.SG4
    {13,  1,  5, 2, "Short FADE Trigger"         },  // ID1.SG6
    {13,  1,  7, 1, "Short ABS"                  },  // ID1.SG8
    {13,  9,  6, 2, "Short OF L1"                },  // ID9.SG7
    {13,  9,  7, 2, "Short OF L2"                },  // ID9.SG8
    // ---- chart #18, 3.0 range ----
    {18,  1, 23, 1, "OF Short v1"                },  // ID1.SG24
    {18,  1, 29, 1, "Bearish Vol Seq"            },  // ID1.SG30
    {18,  1, 31, 2, "Sell 3 Bar Slingshot"       },  // ID1.SG32
    {18,  1, 33, 1, "Delta Trap Short"           },  // ID1.SG34
    {18,  1, 35, 2, "POCS Sell"                  },  // ID1.SG36
    {18,  1, 37, 1, "Sell POC Wave"              },  // ID1.SG38
    // ---- free slots ----
...
// =============================================================================
SCSFExport scsf_OrderflowSignalV5Sell(SCStudyInterfaceRef sc)
{
    // -------------------------------------------------------------------------
    // SUBGRAPHS
    // -------------------------------------------------------------------------
    ...
    if (sc.SetDefaults)
    {
        sc.GraphName        = "Orderflow Signal V5 Sell";
        sc.StudyDescription =
            "V5 SELL: direct cross-chart source reads, short/bearish trigger set. "
            "Rising edge detected in source time, merge-walk mapped to the main "
            "bar, events accumulated. Arrows point down. AutoLoop=0.";
        sc.AutoLoop     = 0;
        sc.GraphRegion  = 1;
        sc.FreeDLL      = 0;
        sc.DrawZeros    = 0;
        ...
        sg_Level1.DrawStyle    = DRAWSTYLE_ARROWDOWN;   // SELL: down
        ...
        sg_Level2.DrawStyle    = DRAWSTYLE_ARROWDOWN;   // SELL: down
        ...
        sg_Level3.DrawStyle    = DRAWSTYLE_ARROWDOWN;   // SELL: down
        ...
        in_TriggerPosition.SetCustomInputIndex(1);   // SELL: Above Candle
        ...
        in_SubpanelMode.SetCustomInputIndex(1);   // screenshot: Yes
        ...
        in_FireTime.SetCustomInputIndex(2);   // screenshot: Hold
        ...
        in_AttribPath.SetString("C:\\SierraChart\\Data\\attribution_v5sell.csv");
    ...
    if (isFullRecalc)
    {
        ...
        if (exportEnabled && attribRows != NULL)
            OFS5WriteAttributionCSV(sc, "short", in_AttribPath, in_AttribAppend,
